...
class OrdersController < ApplicationController
  ...
  def create
    ...
    if @order.start_at < room_handler[:first_available_date]
      @order.errors.add(:start_at, "La date de début du stage ne peut pas être avant #{room_handler[:first_available_date].strftime("%d/%m/%Y")}")
    else
  ...
  def order_params
    params.require(:order).permit(:first_name, :family_name, :email, :address, :start_at, :city, :zip_code, :country, :coupon)
  end
end
